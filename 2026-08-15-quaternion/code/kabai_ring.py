...
PALETTE = [(0x6f, 0x78, 0xc9), (0x8e, 0x7d, 0xc6), (0xac, 0x82, 0xbc),
           (0xc7, 0x89, 0xb1), (0xd7, 0x95, 0xa7), (0x7f, 0x92, 0xd2),
           (0x9e, 0x8f, 0xce), (0xb9, 0x92, 0xc3), (0xd0, 0x9a, 0xb5),
           (0xde, 0xa6, 0xab), (0x90, 0xa7, 0xd9), (0xae, 0xa2, 0xd4),
           (0xc7, 0xa5, 0xc9), (0xda, 0xad, 0xbc), (0xe6, 0xb9, 0xb3)]
...
def draw(centers, path, az, el, shift=(0.0, 0.0, 0.0)):
    ...
    for c in centers:
        for quad, n, idx in rt_faces(c):
            ...
            lam = max(0.0, dot(rn, (0.0, 0.0, 1.0)))*0.35 + \
                  abs(dot(rn, light))*0.65
            base = PALETTE[idx % len(PALETTE)]
            col = "#%02x%02x%02x" % tuple(
                max(0, min(255, int(x*(0.40+0.60*lam) + 42*(1-lam))))
                for x in base)
    ...
    L = ['<svg xmlns="http://www.w3.org/2000/svg" '
         ...
         '<g stroke="#1c1c26" stroke-width="0.014" stroke-linejoin="round">']
    for _, q, col in prep:
