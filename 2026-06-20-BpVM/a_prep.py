...
def _verdict_with_reason(real_d, gap99, abstain, s_list):
    if real_d < 0.9:
        return "BLIND", f"real_d={real_d:.3f}<0.9 (real-collapse)"
    if gap99 <= 0:
        return "BLIND", f"gap99={gap99:+.3f}<=0 (null 未分離)"
    if abstain:
        return "ABSTAIN", f"multibin s split {s_list}"
    return "FORWARD", "clean (real_d>=0.9 & gap99>0 & single-bin s)"
