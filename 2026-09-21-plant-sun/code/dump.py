...
#：桁0と桁3を draw_in で描いた表が基準と一致", table(draw_in(0))==tabs[0], table(draw_in(3))==tabs[0])
import pickle
try: tin=pickle.load(open("/home/claude/prop/tin.pkl","rb"))
except Exception:
    tin=[table(draw_in(-k)) for k in range(1,41)]; pickle.dump(tin,open("/home/claude/prop/tin.pkl","wb"))

print(f"T1 内向きの型紙 k=1..40 で表が基準と一致 {sum(t==tabs[0] for t in tin)}/40")

pickle.dump(dict(tabs=tabs,tin=tin,C2=C2,D0=D0),open("/home/claude/prop/st.pkl","wb"))
